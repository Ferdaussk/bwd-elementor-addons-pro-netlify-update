...
$(window).on('elementor/frontend/init', function () {
    ...
    BackgroundParallax = ModuleHandler.extend({
        ...
        onElementChange: debounce(function (prop) {
            if ((prop.indexOf('section_parallax_') !== -1) || (prop.indexOf('bwdproeb_parallax_') !== -1)) {
                this.run();
            }
        }, 400),
...
;(function($, elementor){
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        BarCode = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if (prop.indexOf('bwdproeb_barcode') !== -1) {
                    this.run();
                }
            }, 400),

            settings: function (key) {
                return this.getElementSettings('bwdproeb_barcode_' + key);
            },
...
; (function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        MegaMenu = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if (prop.indexOf('bwdproeb_megamenu_') !== -1) {
                    this.run();
                }
            }, 400),

            settings: function (key) {
                return this.getElementSettings('bwdproeb_megamenu_' + key);
            },
...
( function( $, elementor ) {
	...
	var widgetCircleMenu = function( $scope, $ ) {
        ...
        $($circleMenu[0]).circleMenu({
            ...
            stbwdproeb_out            : $settings.stbwdproeb_out,
            stbwdproeb_in             : $settings.stbwdproeb_in,
...
(function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function ($) {
        ...
        Confetti = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if (prop.indexOf('bwdproeb_widget_cf_') !== -1) {
                    //  this.instance.reset();
                    this.run();
            ...
            settings: function (key) {
                return this.getElementSettings('bwdproeb_widget_cf_' + key);
            },
...
$(window).on('elementor/frontend/init', function () {
    ...
    CursorEffect = ModuleHandler.extend({
        ...
        onElementChange: debounce(function (prop) {
            if (prop.indexOf('bwdproeb_cursor_effects_') !== -1) {
                this.run();
            }
        }, 400),

        settings: function (key) {
            return this.getElementSettings('bwdproeb_cursor_effects_' + key);
        },
...
;(function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        FloatingEffect = ModuleHandler.extend({
            ...
            settings: function (key) {
                return this.getElementSettings('bwdproeb_floating_effects_' + key);
            },

            onElementChange: debounce(function (prop) {
                if ( prop.indexOf('bwdproeb_floating') !== -1 ) {
...
(function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        Notation = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if (prop.indexOf('bwdproeb_notation_') !== -1) {
                    this.run();
                }
            }, 400),

            settings: function (key) {
                return this.getElementSettings('bwdproeb_notation_' + key);
            },
            ...
            run: function () {
                ...
                $list.forEach(element => {
                    ...
                    if (element.bwdproeb_notation_select_type == 'widget') {
                    ...
                    if (element.bwdproeb_notation_select_type == 'custom') {
                        var customSelector = element.bwdproeb_notation_custom_selector;


                        if (element.bwdproeb_notation_custom_selector && customSelector.length > 1) {
                    ...
                    if (element.bwdproeb_notation_type == 'bracket') {
                        bracketOn = element.bwdproeb_notation_bracket_on;
                    ...
                    if ($selectElement.length > 0) {
                        ...
                        options.type = element.bwdproeb_notation_type;
                        options.color = element.bwdproeb_notation_color || '#f23427';
                        options.animationDuration = element.bwdproeb_notation_anim_duration.size || 800;
                        options.strokeWidth = element.bwdproeb_notation_stroke_width.size || 1;
                        options.rtl = rtl;
                        ...
                        if ($($selectElement).length > 0) {
                            elementorFrontend.waypoint($($element), function () {
                                ...
                                offset: element.bwdproeb_notation_waypoint_offset || 'bottom-in-view',
                                // offset: '90%'
                            });
                        }
        ...
        if (Boolean(elementorFrontend.isEditMode()) === false) {
            ...
            if ($settings.linkWithConfetti === true){
                ...
                jQuery.ajax({
                    ...
                    data: {
                        action: "bwdproeb_connect_confetti",
                        data  : 'empty'
                    },
                    success: function () {
                        //  console.log('done');
...
;(function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        ScrollingEffect = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if ( prop.indexOf('bwdproeb_parallax_effects') !== -1 ) {
                    this.run();
                }
            }, 400),

            settings: function (key) {
                return this.getElementSettings('bwdproeb_parallax_effects_' + key);
            },
...
(function ($, elementor) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        RevealFX = ModuleHandler.extend({
            ...
            settings: function (key) {
                return this.getElementSettings('bwdproeb_reveal_effects_' + key);
            },
            run     : function () {
...
(function ($, elementor) {
    $(window).on('elementor/frontend/init', function () {
        ...
        SoundEffects = ModuleHandler.extend({
            ...
            onElementChange: debounce(function (prop) {
                if (prop.indexOf('bwdproeb_sound_effects_') !== -1) {
                    this.run();
                }
            }, 400),

            settings: function (key) {
                return this.getElementSettings('bwdproeb_sound_effects_' + key);
            },
...
$(window).on('elementor/frontend/init', function () {
    ...
    textReadMore = ModuleHandler.extend({
        ...
        onElementChange: debounce(function (prop) {
            if (prop.indexOf('bwdproeb_text_read_more_') !== -1) {
                this.run();
            }
        }, 400),

        settings: function (key) {
            return this.getElementSettings('bwdproeb_text_read_more_' + key);
        },
...
$(window).on('elementor/frontend/init', function () {
    ...
    ThreedText = ModuleHandler.extend({
        ...
        onElementChange: debounce(function (prop) {
            if (prop.indexOf('bwdproeb_threed_text_') !== -1) {
                this.run();
            }
        }, 400),

        settings: function (key) {
            return this.getElementSettings('bwdproeb_threed_text_' + key);
        },
...
jQuery(document).ready(function () {
    ...
    function bwdproeb_linker_builder(e) {
        var specialChars = "!@#$^&%*()+=-[]/{}|:<>?,.",
            rawText      = e,
    ...
    jQuery(selector).each(function (e) {
        var rawText = jQuery(this).text(),
            url     = bwdproeb_linker_builder(rawText);
        jQuery(this).wrapAll(
            '<a id="ep-hash-link-' +
            e +
            '" data-id="' +
