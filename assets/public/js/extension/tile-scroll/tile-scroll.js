...
$(window).on('elementor/frontend/init', function () {
    ...
    TileScroll = ModuleHandler.extend({
        ...
        //     if (prop.indexOf('bwdproeb_tile_scroll') !== -1) {
        //         this.run();
        //     }
        // }, 400),

        settings: function (key) {
            return this.getElementSettings('bwdproeb_tile_scroll_' + key);
        },
            ...
            if (this.settings('show') == 'yes') {
                if ($('#' + tileScroll_ID).length === 0) {
                    ...
                    this.settings('elements').forEach(element => {
                        let images = element.bwdproeb_tile_scroll_images;

                        let x_start = element.bwdproeb_tile_scroll_x_start.size;
                        let x_end = element.bwdproeb_tile_scroll_x_end.size;
