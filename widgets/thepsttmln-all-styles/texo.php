...
if($thepsttmln_yes_value === $thepsttmln_categories_swtcher):
    ...
    if('show_main_cat' === $thepsttmln_cat_show_status){ 
        ...
        if ( ! empty( $categories ) ):
            echo '<ul class="post-categories"><li><a href="'. esc_url( plugin_dir_url( '../../category/'.'name').$categories[0]->name ) .'" class="thepsttmln-cat">'.esc_html__( $categories[0]->name ).'</a></li></ul>';	
        endif;
...
// author
$thepsttmln_author_icon = '<i class="thepsttmln_icons ' . $settings['thepsttmln_author_icon']['value'] . '"></i>';
